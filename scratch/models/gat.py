    ...
    def forward(
        ...
        if isinstance(node_inputs, tuple):
            ...
            if not hasattr('_fc_src'):
                feat_fc_src = self._fc(
                    node_inputs_src).view(-1, self._num_heads, self._out_feats)
                feat_fc_dst = self._fc(
                    node_inputs_dst).view(-1, self._num_heads, self._out_feats)
            else:
                feat_fc_src = self._fc_src(
                    node_inputs_src).view(-1, self._num_heads, self._out_feats)
                feat_fc_dst = self._fc_dst(
                    node_inputs_dst).view(-1, self._num_heads, self._out_feats)
        else:
            node_inputs_dst = node_inputs

            feat_fc_src = self._fc(
                node_inputs).view(-1, self._num_heads, self._out_feats)
        ...
        if self._attn_fc_dst is not None:
            attn_dst = (
                feat_fc_dst * self._attn_fc_dst).sum(dim=-1).unsqueeze(-1)
